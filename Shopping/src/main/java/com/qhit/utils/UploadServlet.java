        ...
        while(true){
            ...
            if(str.startsWith(startBoundary)){
                ...
                if(position1 == -1){//表示是普通文本域上传
                    ...
                    File path = new File(request.getSession().getServletContext().getRealPath("/")+"images\\upload");
                    if(!path.exists()){
                        path.mkdirs();
                    }
                    DataOutputStream dOutputStream = new DataOutputStream(new FileOutputStream(new File(path.getAbsolutePath()+"\\"+imageName)));
